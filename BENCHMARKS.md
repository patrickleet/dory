...
| On the polling kernel, same-session interleaved bind-npm reached statistical parity with OrbStack. | Sixteen position-balanced interleaved samples per engine across two consecutive 8-round campaigns (`.codex-bench/ab-matched-20260711T152051Z`, `ab-matched-20260711T152206Z`): batch medians Dory 2.285/2.180 s vs OrbStack 2.209/2.145 s (~3% apart), fully overlapping distributions (Dory 2.13-3.04 s, OrbStack 1.98-2.50 s), Dory faster in 6 of 16 head-to-head rounds. Dory ran a 4 vCPU/4 GiB VM against OrbStack's default 12 vCPU/8 GiB (its allocation config did not apply; it measured within 1% of matched earlier the same day), containers identically limited to 2 CPUs/1800 MB. Every sample: exact host/guest trees, one-pass rm. | Publish as a tie under the variance rules — a ~3% delta with overlapping samples is inside run noise, and Dory held the smaller VM. Solo (non-interleaved) medians remain Dory 2.38 s vs OrbStack ~1.97 s; interleaved same-session is the publication protocol. |
